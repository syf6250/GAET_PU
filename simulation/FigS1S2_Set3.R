...
for(jj in 1:length(nseq)){
  ...
  allout[[jj]] <- sumout
  #openxlsx2::write_xlsx(sumout,paste0("fixperf_prop5_DRgen_n",n,".xlsx"))
...
#save(allout,file="sensi_DRgen_Run2000.Rdata")
###plot:
mse.ratio <- t(apply(mse.t,1,function(x){x/min(x)}))
err.ratio <- t(apply(err.t,1,function(x){x/min(x)}))
save(mse.ratio,file="sensi_DRgen_Run2000_mseR.Rdata")
save(err.ratio,file="sensi_DRgen_Run2000_errR.Rdata")
