#' SETM fitting with full parameters
#'
...
#'
#' @return  A list including mixture proportion estimation, parameter estimation, iterations, likelihood and penalized likelihood in SETM.
#' @export
#'
SETMfull=function(posdata,negdata,pival,alp,beta,maxite=1000,eps=1e-4)
{
  ...
  while(err>eps & itenum<=maxite)
  {
    ...
    ind_break <- tryCatch(
      ...
      error = function(e){
        print("ERROR, from SETMfull")
        return(TRUE)
      },
      warning = function(w){
        print(" Warning, from SETMfull")
        return(TRUE)
      })
