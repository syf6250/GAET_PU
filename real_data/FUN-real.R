...
EM.addDR.initcomp2 <- function(seed0,pdata,udata,maxit=1000,thres=1e-4,
  ...
  for(r in 1:maxit){
    ...
    if(pi.est[r+1]<0.01){stop(paste("too small pi occurs at iteration: ",r))}
  ...
  if(dimL>0){
    eval.df2 <- matrix(0,nrow=nrow(eval.df),ncol=dimL)
    colnames(eval.df2) <- names(pdata)[(dimC+1):ncol(pdata)]
    eval.df2 <- as.data.frame(eval.df2)
    eval.df2 <- cbind(eval.df,eval.df2)
  }else{eval.df2 <- eval.df}
  #function estimation
  eval.terms <- predict(gamout,type="terms",newdata = eval.df2)
  if(dimL>0){eval.terms <- eval.terms[,-c(1:dimL),drop=F]}
  ...
  return(list(conv=ifelse(r==maxit,F,T),
              ...
              eval.pts=eval.df,eval.terms=eval.terms,
              init=init.fin))
}
...
EM.addDR.init4.2 <- function(seed0,pdata,udata,maxit=1000,thres=1e-4,
  ...
  for(methodNo in 3:4){###!!!!!!only 2 methods are used now
...
###np bootstrap
npBoot.aic.core <- function(run,init,full.obj,
                            nu.seq,seed,pdata,udata,maxit=1000,thres=1e-4,
                            pi.true=0.4,k=10,ord=3,ord.pen=2,eval.df=NULL,
                            dimC=2,dimL=2,each=1,eval.df.test=NULL){
  #re-sample:
  set.seed(run)
  ...
  if(is.null(init)){
    res <- AIC.addDR2(nu.seq=nu.seq,seed=seed,pdata.selec,udata.selec,maxit=maxit,thres=thres,
                      ...
                      initinput=NULL,dimC=dimC,dimL=dimL,each=each)
  }else{
    #modify initial
    getinit <- init
  ...
  if(isTRUE(res$conv)){
    #need to shift fun est
    shift <- apply(predict(full.obj,type="terms",newdata = X.selec),2,mean)
    if(dimL>0){shift <- shift[-c(1:dimL)]}
    eval.terms <- res$eval.terms+matrix(rep(shift,nrow(res$eval.terms)),
                                        nrow=nrow(res$eval.terms),byrow=T)
    if(is.null(eval.df.test)){
      return(list(conv=T,pi=res$pi,tuneid=res$tuneid,icall=res$icall,eval.terms=eval.terms))
    }else{
      if(dimL>0){
        eval.df.test2 <- matrix(0,nrow=nrow(eval.df.test),ncol=dimL)
        colnames(eval.df.test2) <- names(pdata)[(dimC+1):ncol(pdata)]
        eval.df.test2 <- as.data.frame(eval.df.test2)
        eval.df.test2 <- cbind(eval.df.test,eval.df.test2)
      }else{eval.df.test2 <- eval.df.test}
      eval.terms.te <- predict(res$gamObj,type="terms",newdata = eval.df.test2)
      if(dimL>0){eval.terms.te <- eval.terms.te[,-c(1:dimL),drop=F]}
      eval.terms.te <- eval.terms.te+matrix(rep(shift,nrow(eval.terms.te)),
                                            nrow=nrow(eval.terms.te),byrow=T)
      return(list(conv=T,pi=res$pi,tuneid=res$tuneid,
                  icall=res$icall,eval.terms=eval.terms,eval.terms.test=eval.terms.te))
    }
  }else{return(list(conv=F))}
}


getCIest <- function(test,res.l,dimC,dimL,pdata,
                     eval.df.test=NULL,weights.mat=NULL,bsval.l=NULL,B=NULL){
  #test: AIC.addDR2 output for full data, with one selected tuning
  #res.l: parallel output of npBoot.aic.core
  conv.prop <- mean(sapply(res.l, function(x){x$conv}))
  idall <- do.call(c,lapply(res.l, function(x){x$tuneid}))#useful when init=NULL
  
  #pi
  CI.df <- matrix(nrow=1,ncol=6)#fix tune; 95%
  colnames(CI.df) <- c('M1-L','M1-U','M2-L','M2-U','M3-L','M3-U')
  CI.df.90 <- CI.df
  CI.df.90.logit <- CI.df.logit <- CI.df
  piall <- do.call(c,lapply(res.l, function(x){x$pi}))
  CI.df[1,] <- c(quantile(piall,probs=0.025),quantile(piall,probs=0.975),
                 2*test$pi-quantile(piall,probs=0.975),2*test$pi-quantile(piall,probs=0.025),
                 test$pi-sd(piall)*1.96,test$pi+sd(piall)*1.96)
  CI.df.90[1,] <- c(quantile(piall,probs=0.05),quantile(piall,probs=0.95),
                    2*test$pi-quantile(piall,probs=0.95),2*test$pi-quantile(piall,probs=0.05),
                    test$pi-sd(piall)*1.64,test$pi+sd(piall)*1.64)
  piall.logit <- log(piall/(1-piall))
  piest.logit <- log(test$pi/(1-test$pi))
  CI.df.logit[1,] <- c(quantile(piall.logit,probs=0.025),quantile(piall.logit,probs=0.975),
                       2*piest.logit-quantile(piall.logit,probs=0.975),2*piest.logit-quantile(piall.logit,probs=0.025),
                       piest.logit-sd(piall.logit)*1.96,piest.logit+sd(piall.logit)*1.96)
  CI.df.logit[1,] <- 1/(1+exp(-CI.df.logit[1,]))
  CI.df.90.logit[1,] <- c(quantile(piall.logit,probs=0.05),quantile(piall.logit,probs=0.95),
                          2*piest.logit-quantile(piall.logit,probs=0.95),2*piest.logit-quantile(piall.logit,probs=0.05),
                          piest.logit-sd(piall.logit)*1.64,piest.logit+sd(piall.logit)*1.64)
  CI.df.90.logit[1,] <- 1/(1+exp(-CI.df.90.logit[1,]))
  CI.pi <- rbind(CI.df,CI.df.90,CI.df.logit,CI.df.90.logit)
  rownames(CI.pi) <- c("95","90","95logit","90logit")
  
  #fun
  funall <- list()
  funest <- test$eval.terms
  for(ci in 1:dimC){
    funall[[ci]] <- do.call(rbind,lapply(res.l, function(x){x$eval.terms[,ci]}))
  }
  CI.df.fun.l <- list();CI.df.fun.90.l <- list()
  for(ci in 1:dimC){
    CI.df.fun <- matrix(nrow=length(eval.pts),ncol=6)
    colnames(CI.df.fun) <- c('M1-L','M1-U','M2-L','M2-U','M3-L','M3-U')
    CI.df.fun.90 <- CI.df.fun
    
    quanti <- apply(funall[[ci]],2,quantile,probs=c(0.025,0.975))
    sd.pts <-  apply(funall[[ci]],2,sd)
    CI.df.fun[,1] <- quanti[1,];CI.df.fun[,2] <- quanti[2,]
    CI.df.fun[,3] <- 2*funest[,ci]- quanti[2,];CI.df.fun[,4] <- 2*funest[,ci]- quanti[1,]
    CI.df.fun[,5] <- funest[,ci]-sd.pts*1.96;CI.df.fun[,6] <- funest[,ci]+sd.pts*1.96
    CI.df.fun.l[[ci]] <- CI.df.fun
    
    quanti <- apply(funall[[ci]],2,quantile,probs=c(0.05,0.95))
    CI.df.fun.90[,1] <- quanti[1,];CI.df.fun.90[,2] <- quanti[2,]
    CI.df.fun.90[,3] <- 2*funest[,ci]- quanti[2,];CI.df.fun.90[,4] <- 2*funest[,ci]- quanti[1,]
    CI.df.fun.90[,5] <- funest[,ci]-sd.pts*1.64;CI.df.fun.90[,6] <- funest[,ci]+sd.pts*1.64
    CI.df.fun.90.l[[ci]] <- CI.df.fun.90
  }
  
  #fun test
  if(is.null(eval.df.test)){
    return(list(conv.prop=conv.prop,idall=idall,CI.pi=CI.pi,
                CI.fun95=CI.df.fun.l,CI.fun90=CI.df.fun.90.l))
  }else{
    if(dimL>0){
      eval.df.test2 <- matrix(0,nrow=nrow(eval.df.test),ncol=dimL)
      colnames(eval.df.test2) <- names(pdata)[(dimC+1):ncol(pdata)]
    ...
    eval.terms2 <- predict(test$gamObj,type="terms",newdata = eval.df.test2)
    if(dimL>0){eval.terms2 <- eval.terms2[,-c(1:dimL),drop=F]}
    ...
    for(br in 1:B){
      if(isTRUE(res.l[[br]]$conv)){
        diff.eval <- res.l[[br]]$eval.terms.test-eval.terms2
        for(k in 1:dimC){
          b0sumsq.all[br,k] <- crossprod(as.numeric(weights.mat[,k]%*%(bsval.l[[k]]*as.numeric(diff.eval[,k]))))
        }
      }
    }
    ...
    pval <- numeric(dimC)
    for(k in 1:dimC){
      pval[k] <- (sum(b0sumsq.all[,k]>bsumsq[k],na.rm=T)+1)/(length(which(!is.na(b0sumsq.all[,k])))+1)
    }
    
    return(list(conv.prop=conv.prop,idall=idall,CI.pi=CI.pi,
                CI.fun95=CI.df.fun.l,CI.fun90=CI.df.fun.90.l,
                b0sumsq.all=b0sumsq.all,bsumsq=bsumsq,sigtest=sig,pval=pval))
  }
