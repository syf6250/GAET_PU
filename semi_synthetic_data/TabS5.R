...
source("FUN-para.R")
source("FUN-real.R")
#set various kk for different pi: kk=1,2,3
kk <- 1
...
nodes.mat <- weights.mat <-  matrix(nrow=20,ncol=ncol(raw)-1)
bsval.l <- list()
for(j in 1:(ncol(raw)-1)){
  quantile(raw[,j],prob=c(0.05,0.95))
...
naive.out <- naive.est(dataY1[,c(1:5)],dataY0[,c(1:5)],nu=0.00001,k=10,ord=3,ord.pen=2,dimC=5,dimL=0)


simu.wilt.funbt <- function(run,prob,pi.true,dataY1,dataY0,nu.seq,dimC=5,dimL=0,B=500,
  ...
  ###design integral range and evaluation points(inside the loop)#no linear part!
  colnames(nodes.mat) <- colnames(weights.mat) <- names(pdata)[1:dimC]
  eval.df <- as.data.frame(nodes.mat)
  ...
  if(isTRUE(test$conv)){
    ...
    shift <- apply(predict(naive.out$gamObj,type="terms",newdata = X), 2, mean)
    eval.terms2 <- test$eval.terms+matrix(rep(shift,nrow(test$eval.terms)),
    ...
    for(runB in 1:B){
      ...
      if(isTRUE(res$conv)){
        ...
        shift <- apply(predict(naive.out$gamObj,type="terms",newdata = X.selec),2,mean)
        eval.terms2 <- res$eval.terms+matrix(rep(shift,nrow(res$eval.terms)),
...
output <- foreach(run = 1:RUN,.packages = c("mgcv"))%dopar%{
  simu.wilt.funbt(run,prob=prob,pi.true=pi.true,dataY1=dataY1,dataY0=dataY0,
                  nu.seq=nu.seq,dimC=5,dimL=0,B=500,
